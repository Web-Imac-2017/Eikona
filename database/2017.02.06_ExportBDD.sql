...
CREATE TABLE IF NOT EXISTS `Roger`.`reports` (
  ...
  `report_handler` INT NULL DEFAULT 0,
  `report_result` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`report_id`, `user_id`, `post_id`),
  INDEX `REPORTER_ID_idx` (`user_id` ASC),
  INDEX `REPORTER_POST_idx` (`post_id` ASC),
  INDEX `MODERATOR_ID_idx` (`report_handler` ASC),
  CONSTRAINT `REPORTER_ID`
    FOREIGN KEY (`user_id`)
    REFERENCES `Roger`.`users` (`user_id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `REPORTER_POST`
    FOREIGN KEY (`post_id`)
    REFERENCES `Roger`.`posts` (`post_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `MODERATOR_ID`
    FOREIGN KEY (`report_handler`)
    REFERENCES `Roger`.`users` (`user_id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Roger`.`PARAMS` (
  `PARAM_NAME` TEXT NOT NULL COMMENT 'Paramètre à utiliser sur le site\n	- Nombre maximum de profils\n	- Messages par défaut\n	- etc.',
  `PARAM_VALUE` TEXT NULL DEFAULT NULL,
  `PARAM_edit_time` INT NOT NULL COMMENT '	',
  `PARAM_edit_user_id` INT NULL DEFAULT 0,
  PRIMARY KEY (`PARAM_NAME`),
  INDEX `LASTE_EDIT_MADE_BY_idx` (`PARAM_edit_user_id` ASC),
  CONSTRAINT `LASTE_EDIT_MADE_BY`
    FOREIGN KEY (`PARAM_edit_user_id`)
    REFERENCES `Roger`.`users` (`user_id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE)
ENGINE = InnoDB;
